...
# Ces clés identifient ton application auprès des navigateurs.
# Généra tes propres clés en production avec la commande fournie dans INSTRUCTIONS.md
VAPID_PUBLIC_KEY = "BPYIML9Sjtb8DKZrBeu40DIU2-6g_Vb90770GkIrN852s4SI8V2m1y04yE9kOvuOd-hpcNnkiG4l_HDO76kcBAg"
VAPID_PRIVATE_KEY = "xmNCXLfpRw5hEyP0VaezpRozSToCQwPhpcIWddjU04E"
VAPID_CLAIMS = {"sub": "mailto:armelyoumbi47@gmail.com"}
...
def envoyer_notification(titre, corps):
    """Envoie une notification push à tous les appareils abonnés.
    Supprime automatiquement les abonnements qui ne sont plus valides."""
    if webpush is None:
        return  # pywebpush non installé, on ignore silencieusement

    conn = get_db()
    abonnements = conn.execute("SELECT id, subscription_json FROM abonnements_push").fetchall()

    payload = json.dumps({"title": titre, "body": corps})

    for a in abonnements:
        try:
            webpush(
                subscription_info=json.loads(a["subscription_json"]),
                data=payload,
                vapid_private_key=VAPID_PRIVATE_KEY,
                vapid_claims=dict(VAPID_CLAIMS),
            )
        except WebPushException:
            # L'abonnement n'est plus valide (appli désinstallée, etc.)
            conn.execute("DELETE FROM abonnements_push WHERE id = ?", (a["id"],))
...
def enregistrer_vente():
    ...
    conn.execute("UPDATE produits SET quantite = ? WHERE id = ?", (nouvelle_quantite, produit_id))
    conn.commit()
    conn.close()

    # Notification si le stock vient de franchir le seuil d'alerte, ou tombe à zéro
    ancienne_quantite = produit["quantite"]
    if ancienne_quantite > produit["seuil_alerte"] >= nouvelle_quantite:
        envoyer_notification(
            "⚠️ Stock bas sur Realm",
            f"« {produit['nom']} » : il ne reste que {nouvelle_quantite} unité(s) (seuil : {produit['seuil_alerte']}).",
        )
    elif nouvelle_quantite == 0:
        envoyer_notification(
            "🚨 Rupture de stock sur Realm",
            f"« {produit['nom']} » est en rupture totale de stock.",
        )
...
@app.route("/notifications/cle_publique")
def notifications_cle_publique():
    return jsonify({"cle_publique": VAPID_PUBLIC_KEY})


@app.route("/notifications/abonner", methods=["POST"])
def notifications_abonner():
    subscription = request.get_json(force=True, silent=True)
    if not subscription or "endpoint" not in subscription:
        return jsonify({"ok": False, "erreur": "Abonnement invalide"}), 400

    conn = get_db()
    try:
        conn.execute(
            "INSERT OR REPLACE INTO abonnements_push (endpoint, subscription_json) VALUES (?, ?)",
...
def notifications_test():
    envoyer_notification("🔔 Realm", "Les notifications fonctionnent correctement !")
    return jsonify({"ok": True})
